...
import { CheckCircle, XCircle, MapPin, Briefcase, Clock } from "lucide-react";
        ...
        <DialogHeader className="pb-3 sm:pb-4">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 sm:gap-4">
            ...
            <img
              ...
              className="h-12 w-12 sm:h-16 sm:w-16 rounded-lg object-contain border p-1 flex-shrink-0"
            />
          </div>
        </DialogHeader>
        <div className="mt-3 sm:mt-4 space-y-4 sm:space-y-6">
          <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-4 text-sm text-gray-600">
            <div className="flex items-center gap-2">
              <MapPin className="h-4 w-4 text-gray-400 flex-shrink-0" /> <span>{internship.location}</span>
            </div>
            <div className="flex items-center gap-2">
              {" "}
            ...
            <div className="flex items-center gap-2">
              <Clock className="h-4 w-4 text-gray-400 flex-shrink-0" /> <span>{internship.duration}</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="text-sm text-gray-600">
              <p>
                <strong>Name:</strong> {internship.recruiterName}
              </p>
              <p>
                <strong>Email:</strong> {internship.recruiterEmail}
              </p>
            </div>
          </div>
          ...
          {internship.admin_review && (
            <div className="bg-gray-50 p-3 sm:p-4 rounded-lg">
              ...
              <div className="space-y-2 text-sm">
                <p>
                  <span className="font-medium">Reviewed by:</span> {internship.admin_review.reviewed_by}
                </p>
                <p>
                  <span className="font-medium">Reviewed at:</span>{" "}
                  {new Date(internship.admin_review.reviewed_at).toLocaleDateString()}
                </p>
                {internship.admin_review.comments && (
                  <p>
                    <span className="font-medium">Comments:</span> {internship.admin_review.comments}
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="flex justify-end gap-3 pt-3 sm:pt-4 border-t">
            <Button variant="outline" onClick={() => onOpenChange(false)} className="h-9 sm:h-10 text-sm sm:text-base">
              Close
            </Button>
          </div>
        </div>
